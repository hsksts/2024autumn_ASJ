...
#Size of sound field [m]
lenX=5.0
lenY=0.1

#Interval of sound field [m]
dx=0.05
dy=0.02
...
M = 400 #num of source
N = len(xr) # num of receivers

#sound source position
xs = np.random.uniform(-lenX,lenX,M)#
ys = np.random.uniform(0.1,0.5,M).reshape(M,1)

H_i = np.zeros((N,M),dtype='complex') #incident transfer function
H_r = np.zeros((N,M),dtype='complex') #reflective transfer function
H_r2 = np.zeros((N,M),dtype='complex') #reflective transfer function
Bins = 100 #Number of wavenumber spectrum bin
...
##########################################################################
## Pi, Pr, Cr calculation
##########################################################################
step = 0
F,Finv = fourierMatMN(Bins,Nx)

for m in range(M):
    H_i[:,m] = green2d(Xr[:,step], Yr[:,step], xs[m], ys[m], k)

ys_r = -ys #y cordinate of mirror source
d = 4*c/f/5

for i in range(M):
    H_r[:,i] = green2d(Xr[:,step], Yr[:,step], xs[i], ys_r[i,0], k)
    H_r2[:,i] = green2d(Xr[:,step], Yr[:,step], xs[i], ys_r[i,0]-2*d, k)
...
if condition=='specular':
    pr = 0.5*H_r#specular reflection

elif condition=='bragg':
    pr = 0.5*H_r + 0.5*H_r2#Bragg reflection
...
diff = 2*d*np.cos(theta/180*np.pi) # 行路差
...
for i in range(len(theta)):
    ...
    if condition=='specular':
        pr_theo[:,i] = 0.5*np.exp(-1j*(kx2[-i]*x+ky2[-i]*y))#specular reflection
    elif condition=='bragg':
        pr_theo[:,i] = 0.5*np.exp(-1j*(kx2[-i]*x+ky2[-i]*y))+0.5*np.exp(-1j*(kx2[-i]*x+ky2[-i]*y))*np.exp(1j*k2*diff[i])#bragg reflection
    else:
        print('condition error')
...
diff = 2*d*np.cos(theta_calc/180*np.pi) # 行路差
for i in range(len(theta_calc)):
    pi_theo_sim[:,i] = 1.0*np.exp(-1j*(kx[i]*x+ky[i]*y))
    # pr_theo_sim[:,i] = 0.5*np.exp(-1j*(kx[-i]*x+ky[-i]*y))#specular reflection
    pr_theo_sim[:,i] = 0.5*np.exp(-1j*(kx[-i]*x+ky[-i]*y))+0.5*np.exp(-1j*(kx[-i]*x+ky[-i]*y))*np.exp(1j*k*diff[i])#bragg reflection
